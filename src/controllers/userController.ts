...
export const changePasswordController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!oldPassword || !newPassword) {
            next( CustomError.badRequest('Старый и новый пароли должны быть предоставлены'));
        }
        await changePassword(userId, oldPassword, newPassword);
        return res.status(200).json({ status: 'success', message: 'Пароль успешно изменен' });
    } catch (e: any) {
        if (e.name === 'UnauthorizedError') {
            next(CustomError.unauthorized(e.message, e));
        } else if (e.name === 'NotFoundError') {
            next(CustomError.notFound(e.message, e));
        } else {
            next(CustomError.internal('Ошибка смены пароля', e));
        }
    }
};
